...
function do_post_message(){
	...
	$email = check_param('email','string',$_SESSION,$nojson);
	$threadID = check_param('threadID','int',$_REQUEST,$nojson)
	// Will have to pull additional information 

	$sql = "
		INSERT INTO  messages(threadID,fileNamePath,creator) 
		VALUES($threadID,'$fileNamePath','$creator') 
	";
...
function do_post_file(){
	$text = check_param('text','string');
	$email = check_param('email','string',$_SESSION,$nojson);
	$threadID = check_param('threadID','int',$_REQUEST,$nojson)
	// Will have to pull additional information 

	$sql = "
		INSERT INTO  messages(threadID,text,creator) 
		VALUES($threadID,'$text','$creator') 
	";
	$result = check_result($sql, $nojson);
